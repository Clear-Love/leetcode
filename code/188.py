...
class Solution:
    def maxProfit(self, k: int, prices: List[int]) -> int:
        ...
        for i, price in enumerate(prices[1:], start=1):
            ...
            dp[i][1][1] = max(dp[i-1][1][1], dp[i-1][1][0] + price)
            for cnt in range(1, k):
                # 第cnt次买
                dp[i][0][cnt] = max(dp[i-1][0][cnt], dp[i-1][0][cnt] - price)
                # 第cnt次卖
                dp[i][1][cnt] = max(dp[i-1][1][cnt], dp[i-1][0][cnt-1] + price)
        return dp[-1][-1][-1]